...
import { EVENT_STICKY_NOTES_REACTION_ADDED } from '~/types/events'
...
const reactToStickyNote = async (params: { userId: string, stickyNoteId: string, reaction: string }) => {
  ...
  const { data, error } = await supabase.from('sticky_notes_reactions')
    .upsert({ reaction, sticky_note_id: stickyNoteId, user_id: userId }, {
